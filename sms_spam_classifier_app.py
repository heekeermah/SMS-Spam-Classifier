...
Original file is located at
    https://colab.research.google.com/drive/1lPyu_7oGrFV7m9gGf_-VwCnzABiSPw5M
"""
...
# These downloads are required for the preprocessing steps
try:
...
def preprocess_text(text):
    ...
    for word in words:
        if word.isalpha() and word not in stop_words: # Keep only alphabetic and non-stopwords
            # 5. Lemmatization
            # The notebook uses wordnet_lemmatizer.lemmatize(word) after stopword removal
...
if st.button("Classify"):
    if not user_input or user_input.strip() == "":
        ...
        try:
            ...
            # Retrieve class labels from the encoder (assuming you saved it or know the mapping)
            # If 'ham' is 0 and 'spam' is 1, then:
            predicted_label = "spam" if prediction[0] == 1 else "NOT SPAM"

            # Optional: Display probabilities
            # prediction_proba = model.predict_proba(input_vector)
            # spam_prob = prediction_proba[0][vectorizer.classes_.tolist().index('spam')] # This won't work directly with CountVectorizer output unless classes_ are set.
            # You might need to map target labels to actual text if encoder is not saved.
            # For MultinomialNB, the order of classes in `model.classes_` will determine which probability corresponds to which label.
            # If your LabelEncoder maps 'ham' to 0 and 'spam' to 1:
            # spam_probability = prediction_proba[0][1] # Probability of being spam
